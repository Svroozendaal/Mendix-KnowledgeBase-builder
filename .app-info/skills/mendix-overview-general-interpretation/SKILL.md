...
1. `<run-folder>/general/app-info.pseudo.txt` + `.json` — app metadata, summary stats, security level
2. `<run-folder>/general/user-roles.pseudo.txt` + `.json` — user roles with module role mappings
3. `<run-folder>/general/all-modules.pseudo.txt` + `.json` — all modules with categories, roles, counts
4. `<run-folder>/general/marketplace-modules.pseudo.txt` + `.json` — marketplace modules subset
5. `<run-folder>/manifest.json` — artifact inventory and generation metadata
6. `<run-folder>/modules/*/flows.pseudo.txt` — module flow files (for call graph extraction)
7. `<run-folder>/modules/*/domain-model.pseudo.txt` — module domain models (for access rule synthesis)

## Workflow

1. **Parse app summary** from `app-info` — extract module count, entity/association/flow totals, security level, admin user, guest access settings.
...
5. **Write app-level Markdown** to KB output:
   - `APP_OVERVIEW.md` — app identity, security level, summary stats, source metadata
   - `MODULE_LANDSCAPE.md` — module table with categories, roles, complexity, and brief purpose notes
   - `SECURITY.md` — user roles, module role mappings, access patterns, XPath constraints explained
   - `CALL_GRAPH.md` — cross-module dependency map, high-connectivity flows, module clusters
...
### Most called (fan-in)
- [flow name] — called by [n] flows from [modules]

### Most calling (fan-out)
- [flow name] — calls [n] other flows
...
1. Keep quantitative facts aligned with exported counts — do not invent numbers.
...
4. Do not duplicate full raw exports inside Markdown — summarise and point to source files.
