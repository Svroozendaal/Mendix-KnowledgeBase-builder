...
## Generated KB Root Layout

The current runtime writes one self-contained KB root at:

`mendix-data/knowledge-base/`

The required markdown contract inside that root is:
...
## Additive Runtime Artifacts

The KB may also contain additive operational files that do not change the required markdown contract:

1. `CLAUDE.md`:
   - AI bootstrap entry point shipped with generated KBs.
2. `.agents/`:
   - AI interpretation agents and skills bundled with the generated KB.
3. `_sources/creator-link.json`:
   - links the KB back to the creator package and last run folder for `/enrichkb` and `/initkb`.
4. `_sources/INITKB_HANDOFF.md`:
   - generated handoff instructions for phase-2 enrichment and revalidation.
5. `_reports/UNKNOWN_TODO.md`:
   - unresolved evidence gaps detected during composition.
6. `_reports/semantic-benchmark.md`:
   - benchmark report generated by semantic validation.
7. `_reports/APP_CONTEXT_OVERLAY.md` and `_reports/APP_CONTEXT_TODO.md`:
   - optional post-KB context conversation outputs defined in document `11`.

Rule: additive runtime artifacts may be added, but the required KB contract above must remain stable unless the KB format version changes.
