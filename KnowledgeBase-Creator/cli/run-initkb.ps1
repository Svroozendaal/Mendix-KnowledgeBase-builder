...
$runDumpParserScript = Join-Path $cliRoot "run-dump-parser.ps1"
...
function Write-HandoffFile {
    ...
    $contentLines = @(
        ...
        '3. `.agents/skills/enrichkb/SKILL.md`',
        "",
        ...
        "",
        '```powershell',
        ".\cli\run-kb-scaffold.ps1 -Validate -OutputRoot `"$ResolvedKnowledgeBaseRoot`" -AppName `"$ResolvedAppName`"",
        ".\cli\run-kb-quality-gate.ps1 -OutputRoot `"$ResolvedKnowledgeBaseRoot`" -AppName `"$ResolvedAppName`"",
        '```',
        "",
        "## Ready prompt",
        "",
        ...
        "3. $packageRoot\.agents\skills\enrichkb\SKILL.md",
        "",
