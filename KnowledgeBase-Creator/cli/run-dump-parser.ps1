...
. (Join-Path $cliRoot "lib/app-overview-resolver.ps1")
...
function Write-CreatorLinkFile {
    ...
    $payload = [ordered]@{
        ...
        creatorInitkbRunner = (Join-Path $cliRoot "run-initkb.ps1")
        appName = $AppName
...
function Write-InitKbHandoffFile {
    ...
    $contentLines = @(
        ...
        "- Creator runner: $(Join-Path $cliRoot 'run-initkb.ps1')",
        "- Data root: $DataRoot",
        ...
        "",
        '```powershell',
        ".\cli\run-kb-scaffold.ps1 -Validate -OutputRoot `"$KbRoot`" -AppName `"$AppName`"",
        ".\cli\run-kb-quality-gate.ps1 -OutputRoot `"$KbRoot`" -AppName `"$AppName`"",
        '```'
    )
...
$scaffoldScript = Join-Path $cliRoot "run-kb-scaffold.ps1"
$composeScript = Join-Path $cliRoot "run-kb-compose.ps1"
$qualityGateScript = Join-Path $cliRoot "run-kb-quality-gate.ps1"
$semanticBenchmarkScript = Join-Path $cliRoot "run-kb-semantic-benchmark.ps1"
$mxCliFullRunScript = Join-Path $cliRoot "run-mxcli-json-v2-full-run.ps1"
