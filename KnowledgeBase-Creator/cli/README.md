# KnowledgeBase Creator CLI

This folder contains the standalone CLI runners and shared PowerShell helpers for the KnowledgeBase creator pipeline.

## Start

Run the end-to-end creator flow from the `KnowledgeBase-Creator/` package root:

```powershell
.\cli\run-initkb.ps1
```

Run the extraction, scaffold, compose, validation, and benchmark pipeline directly:
...
All executable pipeline scripts, including `run-initkb.ps1`, live in `cli/`.
