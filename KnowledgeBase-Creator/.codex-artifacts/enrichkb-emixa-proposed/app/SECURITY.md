...
- Administrator: Operates as the platform owner for inspection and notification domains, including broad maintenance and oversight pages.
- Inspector: Executes day-to-day operational work, including task handling, equipment visibility, and notification consumption.
- Manager: Oversees inspector execution, creates and edits key domain data, and has broader edit rights on operational records.
- Anonymous: Limited pre-authentication access, mainly for registration and login entry.
...
## XPath Constraint Interpretation

The XPath constraints restrict inspector visibility to records connected to their own user account. In practice, this means inspectors can only see tasks and inspection items that are linked through `Inspection.Task_Inspector -> Inspection.Inspector -> Inspection.Inspector_Account = CurrentUser`, which enforces per-inspector work isolation.
